	.file	"lcd_aip31068_i2c.c"
__SP_H__ = 0x3e
__SP_L__ = 0x3d
__SREG__ = 0x3f
__tmp_reg__ = 0
__zero_reg__ = 1
	.text
.global	LCD_Aip31068_SendCommand
	.type	LCD_Aip31068_SendCommand, @function
LCD_Aip31068_SendCommand:
	push r28
	push r29
/* prologue: function */
/* frame size = 0 */
/* stack size = 2 */
.L__stack_usage = 2
	sbiw r24,0
	brne .L2
.L7:
	ldi r28,lo8(1)
	ldi r29,0
.L1:
	movw r24,r28
/* epilogue start */
	pop r29
	pop r28
	ret
.L2:
	mov r28,r22
	ldi r22,0
	movw r30,r24
	ld r24,Z
	call I2C_WriteAddress
	or r24,r25
	brne .L7
	ldi r24,0
	call I2C_WriteData
	or r24,r25
	breq .L5
.L6:
	call I2C_Stop
	rjmp .L7
.L5:
	mov r24,r28
	call I2C_WriteData
	movw r28,r24
	or r24,r25
	brne .L6
	call I2C_Stop
	rjmp .L1
	.size	LCD_Aip31068_SendCommand, .-LCD_Aip31068_SendCommand
.global	LCD_Aip31068_WriteChar
	.type	LCD_Aip31068_WriteChar, @function
LCD_Aip31068_WriteChar:
	push r28
	push r29
/* prologue: function */
/* frame size = 0 */
/* stack size = 2 */
.L__stack_usage = 2
	sbiw r24,0
	brne .L9
.L14:
	ldi r28,lo8(1)
	ldi r29,0
.L8:
	movw r24,r28
/* epilogue start */
	pop r29
	pop r28
	ret
.L9:
	mov r28,r22
	ldi r22,0
	movw r30,r24
	ld r24,Z
	call I2C_WriteAddress
	or r24,r25
	brne .L14
	ldi r24,lo8(64)
	call I2C_WriteData
	or r24,r25
	breq .L12
.L13:
	call I2C_Stop
	rjmp .L14
.L12:
	mov r24,r28
	call I2C_WriteData
	movw r28,r24
	or r24,r25
	brne .L13
	call I2C_Stop
	rjmp .L8
	.size	LCD_Aip31068_WriteChar, .-LCD_Aip31068_WriteChar
.global	LCD_Aip31068_WriteString
	.type	LCD_Aip31068_WriteString, @function
LCD_Aip31068_WriteString:
	push r16
	push r17
	push r28
	push r29
/* prologue: function */
/* frame size = 0 */
/* stack size = 4 */
.L__stack_usage = 4
	sbiw r24,0
	brne .L16
.L27:
	ldi r16,lo8(1)
	ldi r17,0
.L15:
	movw r24,r16
/* epilogue start */
	pop r29
	pop r28
	pop r17
	pop r16
	ret
.L16:
	cp r22,__zero_reg__
	cpc r23,__zero_reg__
	breq .L27
	movw r28,r22
	ldi r22,0
	movw r30,r24
	ld r24,Z
	call I2C_WriteAddress
	or r24,r25
	brne .L27
	ldi r24,lo8(64)
	call I2C_WriteData
	movw r16,r24
	or r24,r25
	brne .L19
.L20:
	ld r24,Y+
	cpse r24,__zero_reg__
	rjmp .L21
	call I2C_Stop
	rjmp .L15
.L21:
	call I2C_WriteData
	or r24,r25
	breq .L20
.L19:
	call I2C_Stop
	rjmp .L27
	.size	LCD_Aip31068_WriteString, .-LCD_Aip31068_WriteString
.global	LCD_Aip31068_SetCursor
	.type	LCD_Aip31068_SetCursor, @function
LCD_Aip31068_SetCursor:
	push r28
	push r29
	rcall .
	in r28,__SP_L__
	in r29,__SP_H__
/* prologue: function */
/* frame size = 2 */
/* stack size = 4 */
.L__stack_usage = 4
	std Y+1,__zero_reg__
	ldi r18,lo8(64)
	std Y+2,r18
	sbiw r24,0
	breq .L28
	movw r30,r24
	ldd r18,Z+1
	cp r22,r18
	brsh .L28
	ldd r18,Z+2
	cp r20,r18
	brsh .L28
	movw r30,r28
	add r30,r22
	adc r31,__zero_reg__
	ldd r22,Z+1
	add r22,r20
	ori r22,lo8(-128)
/* epilogue start */
	pop __tmp_reg__
	pop __tmp_reg__
	pop r29
	pop r28
	jmp LCD_Aip31068_SendCommand
.L28:
	ldi r24,lo8(1)
	ldi r25,0
/* epilogue start */
	pop __tmp_reg__
	pop __tmp_reg__
	pop r29
	pop r28
	ret
	.size	LCD_Aip31068_SetCursor, .-LCD_Aip31068_SetCursor
.global	LCD_Aip31068_WriteStringAt
	.type	LCD_Aip31068_WriteStringAt, @function
LCD_Aip31068_WriteStringAt:
	push r16
	push r17
	push r28
	push r29
/* prologue: function */
/* frame size = 0 */
/* stack size = 4 */
.L__stack_usage = 4
	movw r28,r24
	movw r16,r18
	call LCD_Aip31068_SetCursor
	or r24,r25
	brne .L35
	movw r22,r16
	movw r24,r28
/* epilogue start */
	pop r29
	pop r28
	pop r17
	pop r16
	jmp LCD_Aip31068_WriteString
.L35:
	ldi r24,lo8(1)
	ldi r25,0
/* epilogue start */
	pop r29
	pop r28
	pop r17
	pop r16
	ret
	.size	LCD_Aip31068_WriteStringAt, .-LCD_Aip31068_WriteStringAt
.global	LCD_Aip31068_Clear
	.type	LCD_Aip31068_Clear, @function
LCD_Aip31068_Clear:
/* prologue: function */
/* frame size = 0 */
/* stack size = 0 */
.L__stack_usage = 0
	ldi r22,lo8(1)
	jmp LCD_Aip31068_SendCommand
	.size	LCD_Aip31068_Clear, .-LCD_Aip31068_Clear
.global	LCD_Aip31068_Home
	.type	LCD_Aip31068_Home, @function
LCD_Aip31068_Home:
/* prologue: function */
/* frame size = 0 */
/* stack size = 0 */
.L__stack_usage = 0
	ldi r22,lo8(2)
	jmp LCD_Aip31068_SendCommand
	.size	LCD_Aip31068_Home, .-LCD_Aip31068_Home
.global	LCD_Aip31068_DisplayOnOff
	.type	LCD_Aip31068_DisplayOnOff, @function
LCD_Aip31068_DisplayOnOff:
/* prologue: function */
/* frame size = 0 */
/* stack size = 0 */
.L__stack_usage = 0
	movw r18,r24
	lds r25,g_displayControl
	tst r22
	breq .L39
	ori r25,lo8(4)
.L41:
	sts g_displayControl,r25
	lds r22,g_displayControl
	ori r22,lo8(8)
	movw r24,r18
	jmp LCD_Aip31068_SendCommand
.L39:
	andi r25,lo8(-5)
	rjmp .L41
	.size	LCD_Aip31068_DisplayOnOff, .-LCD_Aip31068_DisplayOnOff
.global	LCD_Aip31068_Init
	.type	LCD_Aip31068_Init, @function
LCD_Aip31068_Init:
	push r28
	push r29
/* prologue: function */
/* frame size = 0 */
/* stack size = 2 */
.L__stack_usage = 2
	sbiw r24,0
	brne .L43
.L45:
	ldi r24,lo8(1)
	ldi r25,0
.L42:
/* epilogue start */
	pop r29
	pop r28
	ret
.L43:
	movw r28,r24
	ldi r22,lo8(56)
	call LCD_Aip31068_SendCommand
	or r24,r25
	brne .L45
	ldi r24,lo8(12)
	sts g_displayControl,r24
	ldi r22,lo8(12)
	movw r24,r28
	call LCD_Aip31068_SendCommand
	or r24,r25
	brne .L45
	movw r24,r28
	call LCD_Aip31068_Clear
	or r24,r25
	brne .L45
	ldi r22,lo8(6)
	movw r24,r28
	call LCD_Aip31068_SendCommand
	ldi r19,lo8(1)
	ldi r18,0
	or r24,r25
	brne .L46
	ldi r19,0
.L46:
	mov r24,r19
	mov r25,r18
	rjmp .L42
	.size	LCD_Aip31068_Init, .-LCD_Aip31068_Init
.global	LCD_InitDefault
	.type	LCD_InitDefault, @function
LCD_InitDefault:
/* prologue: function */
/* frame size = 0 */
/* stack size = 0 */
.L__stack_usage = 0
	ldi r30,lo8(g_lcdHandle)
	ldi r31,hi8(g_lcdHandle)
	ldi r24,lo8(62)
	st Z,r24
	ldi r24,lo8(2)
	std Z+1,r24
	ldi r24,lo8(16)
	std Z+2,r24
	movw r24,r30
	jmp LCD_Aip31068_Init
	.size	LCD_InitDefault, .-LCD_InitDefault
	.section	.rodata.str1.1,"aMS",@progbits,1
.LC0:
	.string	"SET%-4d ACT%-4d%c"
.LC1:
	.string	"%3d%% %2d.%dA %2dV %3dC"
	.text
.global	LCD_Update
	.type	LCD_Update, @function
LCD_Update:
	push r14
	push r15
	push r16
	push r17
	push r28
	push r29
	in r28,__SP_L__
	in r29,__SP_H__
	sbiw r28,34
	in __tmp_reg__,__SREG__
	cli
	out __SP_H__,r29
	out __SREG__,__tmp_reg__
	out __SP_L__,r28
/* prologue: function */
/* frame size = 34 */
/* stack size = 40 */
.L__stack_usage = 40
	movw r30,r24
	ldi r24,lo8(1)
	ldi r25,0
	sbiw r30,0
	brne .+2
	rjmp .L48
	ldd r24,Z+20
	ldd r25,Z+21
	cpi r24,1
	cpc r25,__zero_reg__
	brne .+2
	rjmp .L54
	sbiw r24,2
	breq .+2
	rjmp .L55
	ldi r25,lo8(82)
.L51:
	movw r16,r30
	push __zero_reg__
	push r25
	ldd r24,Z+5
	push r24
	ldd r24,Z+4
	push r24
	ldd r24,Z+3
	push r24
	ldd r24,Z+2
	push r24
	ldi r24,lo8(.LC0)
	ldi r25,hi8(.LC0)
	push r25
	push r24
	movw r14,r28
	ldi r24,18
	add r14,r24
	adc r15,__zero_reg__
	push r15
	push r14
	call sprintf
	movw r26,r16
	adiw r26,11
	ld r30,X+
	ld r31,X
	sbiw r26,11+1
	adiw r26,15
	ld r24,X
	sbiw r26,15
	push __zero_reg__
	push r24
	adiw r26,13
	ld r24,X+
	ld r25,X
	ldi r18,lo8(-24)
	ldi r19,lo8(3)
	movw r22,r18
	call __udivmodhi4
	push r23
	push r22
	movw r24,r30
	ldi r22,lo8(100)
	ldi r23,0
	call __udivmodhi4
	movw r24,r22
	ldi r22,lo8(10)
	ldi r23,0
	call __udivmodhi4
	push r25
	push r24
	movw r24,r30
	movw r22,r18
	call __udivmodhi4
	push r23
	push r22
	movw r30,r16
	ldd r24,Z+10
	push __zero_reg__
	push r24
	ldi r24,lo8(.LC1)
	ldi r25,hi8(.LC1)
	push r25
	push r24
	movw r16,r28
	subi r16,-1
	sbci r17,-1
	push r17
	push r16
	call sprintf
	movw r18,r14
	ldi r20,0
	ldi r22,0
	ldi r24,lo8(g_lcdHandle)
	ldi r25,hi8(g_lcdHandle)
	call LCD_Aip31068_WriteStringAt
	movw r18,r16
	ldi r20,0
	ldi r22,lo8(1)
	ldi r24,lo8(g_lcdHandle)
	ldi r25,hi8(g_lcdHandle)
	call LCD_Aip31068_WriteStringAt
	in __tmp_reg__,__SREG__
	cli
	out __SP_H__,r29
	out __SREG__,__tmp_reg__
	out __SP_L__,r28
	ldi r25,0
	ldi r24,0
.L48:
/* epilogue start */
	adiw r28,34
	in __tmp_reg__,__SREG__
	cli
	out __SP_H__,r29
	out __SREG__,__tmp_reg__
	out __SP_L__,r28
	pop r29
	pop r28
	pop r17
	pop r16
	pop r15
	pop r14
	ret
.L55:
	ldi r25,lo8(45)
	rjmp .L51
.L54:
	ldi r25,lo8(70)
	rjmp .L51
	.size	LCD_Update, .-LCD_Update
	.section	.rodata.str1.1
.LC2:
	.string	"!! TRIPPED !!   "
.LC3:
	.string	"!TRIP CODE=%d"
	.text
.global	LCD_ShowTrip
	.type	LCD_ShowTrip, @function
LCD_ShowTrip:
	push r16
	push r17
	push r28
	push r29
	in r28,__SP_L__
	in r29,__SP_H__
	sbiw r28,17
	in __tmp_reg__,__SREG__
	cli
	out __SP_H__,r29
	out __SREG__,__tmp_reg__
	out __SP_L__,r28
/* prologue: function */
/* frame size = 17 */
/* stack size = 21 */
.L__stack_usage = 21
	mov r17,r24
	mov r16,r25
	lds r24,blinkCounter.1854
	lds r25,blinkCounter.1854+1
	adiw r24,1
	cpi r24,15
	cpc r25,__zero_reg__
	brsh .L57
	sts blinkCounter.1854+1,r25
	sts blinkCounter.1854,r24
.L58:
	ldi r18,lo8(.LC2)
	ldi r19,hi8(.LC2)
	ldi r20,0
	ldi r22,0
	ldi r24,lo8(g_lcdHandle)
	ldi r25,hi8(g_lcdHandle)
	call LCD_Aip31068_WriteStringAt
	lds r24,blinkState.1855
	tst r24
	breq .L60
	push r16
	push r17
	ldi r24,lo8(.LC3)
	ldi r25,hi8(.LC3)
	push r25
	push r24
	movw r16,r28
	subi r16,-1
	sbci r17,-1
	push r17
	push r16
	call sprintf
	movw r18,r16
	ldi r20,0
	ldi r22,lo8(1)
	ldi r24,lo8(g_lcdHandle)
	ldi r25,hi8(g_lcdHandle)
	call LCD_Aip31068_WriteStringAt
	pop __tmp_reg__
	pop __tmp_reg__
	pop __tmp_reg__
	pop __tmp_reg__
	pop __tmp_reg__
	pop __tmp_reg__
.L61:
	ldi r25,0
	ldi r24,0
/* epilogue start */
	adiw r28,17
	in __tmp_reg__,__SREG__
	cli
	out __SP_H__,r29
	out __SREG__,__tmp_reg__
	out __SP_L__,r28
	pop r29
	pop r28
	pop r17
	pop r16
	ret
.L57:
	sts blinkCounter.1854+1,__zero_reg__
	sts blinkCounter.1854,__zero_reg__
	ldi r24,lo8(1)
	lds r25,blinkState.1855
	cpse r25,__zero_reg__
	ldi r24,0
.L59:
	sts blinkState.1855,r24
	rjmp .L58
.L60:
	ldi r24,lo8(g_driveData)
	ldi r25,hi8(g_driveData)
	call LCD_Update
	rjmp .L61
	.size	LCD_ShowTrip, .-LCD_ShowTrip
	.local	blinkState.1855
	.comm	blinkState.1855,1,1
	.local	blinkCounter.1854
	.comm	blinkCounter.1854,2,1
	.data
	.type	g_displayControl, @object
	.size	g_displayControl, 1
g_displayControl:
	.byte	12
	.local	g_lcdHandle
	.comm	g_lcdHandle,8,1
	.ident	"GCC: (GNU) 7.3.0"
.global __do_copy_data
.global __do_clear_bss
